...
- `figures/ml_baseline_confusion_matrix.png`
...
- `bc_pytorch/train_bc.py`
- `figures/bc_training_curve.png`
- `checkpoints/bc_policy.pt`
- `reports/bc_pytorch_result.md`
- `notes/03_bc_pytorch_notes.md`
...
当前状态：**Stage 7 最终交付整理中**。本阶段不再新增技术功能，按以下顺序收束：

- Stage 7A：项目总审计，已完成。
- Stage 7B：文档整理与复现说明统一，已完成。
- Stage 7C：老师汇报材料准备，已完成 Markdown 版材料。
- Stage 7D：最终交付检查进行中；正式 PPT/PDF 需人工排版与确认。

### 目标

统一项目文档、结果表述和复现说明，再准备最终汇报材料。Stage 7 是整理阶段，不进入 CNN、DQN、ROS/Gazebo 等扩展实现。
...
- Stage 6：C++ A* 工程实现已完成。
- Stage 7：最终交付整理中；Stage 7A、7B 已完成，Stage 7C 已形成 Markdown 版材料，Stage 7D 进行最终人工检查。
...
Stage 6 已使用 C++17 重新实现 A*，并在与 Python demo 相同的地图上完成构建、运行和路径合法性验证。

当前推荐先人工检查仓库文档与汇报大纲，再根据老师沟通场景制作最终 PPT/PDF。Stage 7 期间不新增技术功能。
