...
- `rl_qlearning/evaluate_qlearning.py`
- `figures/qlearning_reward_curve.png`
- `figures/qlearning_path.png`
...
- Stage 4：PyTorch Behavior Cloning 尚未开始。

Stage 3 使用八个 state 特征预测 `action`，按 `map_id` 划分训练集和测试集，并比较 KNN、Logistic Regression 和 Decision Tree。当前结果中 Logistic Regression 的 single-step action accuracy 最好。

Stage 3 已确认代码可以复现、结果文件完整、笔记能够解释模型和指标、文档与实际进度一致。下一阶段是 Stage 4，但当前尚未开始。

Stage 3 尚未进行完整导航 rollout。当前结果只能说明模型在测试样本上的单步动作预测表现，不能表述为 Logistic Regression 已经会导航。本阶段收尾不进入 PyTorch、强化学习或 rollout。
