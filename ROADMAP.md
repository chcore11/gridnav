...
Stage 4.5 已使用 Stage 4 BC policy 完成 closed-loop rollout evaluation。这个阶段只评估已有策略，不使用 reward，也不更新策略，因此不是强化学习。

Stage 5 Q-learning 将通过 reward 与环境交互并学习 `Q(s,a)`，这是下一阶段才会开始的强化学习任务。
